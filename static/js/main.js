...
(function ($) {
    ...
    try {
        ...
        toastr.options.positionClass = 'toast-top-center'; // Centra el mensaje

        $(document).on('click', 'a.company', function () {
            let company_id = $(this).attr("pk");
            $.ajax({
                url: '/hrm/change_companies/',
                async: true,
                ...
                success: function (response, textStatus, xhr) {
                    if (xhr.status === 200) {
                        if (response.companyID === 1) {
                            toastr.success(response.company, '', {
                                "positionClass": "toast-top-center",
                                "toastClass": "custom-toast"
                            });
                            document.querySelector('.header-desktop3').style.setProperty('background-color', '#0079c5', 'important');
                        } else {
                            toastr.success(response.company, '', {
                                "positionClass": "toast-top-center",
                            });
                            document.querySelector('.header-desktop3').style.setProperty('background-color', '#722b2b', 'important');
                        }
                        document.getElementById("selected_company").innerText = response.short_name;
                        document.getElementById('selected_company').setAttribute('company', response.companyID);
                        //getSerialAndCorrelative('B')
                        /*setTimeout(() => {
                            window.location.href = "/comercial/programming_list/";
                        }, 100);*/
                    }
                },
